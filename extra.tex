...
\noi $9.$ Find two compact sets with noncomact intersection.

\ 

\noi $10.$ Let $K$ be a compact space.
Show that a function $f\:X\to K$ is continuous if and only if its graph $\set{(x,f(x))\in X\times K}{x\in X}$ is closed. 
...
\noi $14.$ Let $X$ be a topological space, $V, W \subset  X$ two open subsets. 
Prove that if
